...
int main (int argc, char *argv[])
{
    ...
    ind=5; start=0; N=500; rho=0.7; set_tem=0.8;
...
double tau=0.5, dt=0.005;
    ...
    for(int i=0; i<N; i++)
   {
    ...
    sumv.z /= double(N);
    vsq=vsq/ double(N);
    temp_init=vsq/double(3);

  std::ofstream outfil;
  outfil.open("test.txt", std::ios::app);   // text file to view T and temp_init at every iteration.
  outfil<<"\t"<<T<<"\t"<<temp_init<<"\n";

    scale = sqrt( double(1)+ (dt/tau)*((T/temp_init)- double(1)));
    sumvn.x = 0.0;
    sumvn.y = 0.0;
    sumvn.z = 0.0;
        for(int i=0; i<N; i++)
